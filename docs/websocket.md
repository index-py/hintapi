...
```python
from typedweb import websocket
...
与 HTTP 类处理器类似，WebSocket 类处理器可以从 `typedweb.SocketView` 继承而来。
...
每个 WebSocket 连接都会对应一个 `typedweb.requests.WebSocket` 对象，它拥有一对 `receive`/`send` 函数。但为了方便使用，在此基础上封装了三对 recv/send 函数。
